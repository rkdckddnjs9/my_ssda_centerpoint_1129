...
class CenterHeadRCNNV2(nn.Module):
    ...
    def get_loss_ssl(self, scalar=True):
        cls_loss, tb_dict = self.get_cls_layer_loss_ssl(scalar=scalar)
        #cls_loss, tb_dict = self.get_cls_layer_loss()
        box_loss, tb_dict_box = self.get_box_reg_layer_loss_ssl(scalar=scalar)
        tb_dict.update(tb_dict_box)
        #rpn_loss = cls_loss + box_loss
