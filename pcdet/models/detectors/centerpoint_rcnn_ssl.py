...
class CenterPointRCNN_SSL(Detector3DTemplate):
    def __init__(self, model_cfg, num_class, dataset):
        super().__init__(model_cfg=model_cfg, num_class=num_class, dataset=dataset)
        # something changes so need deep copy
        model_cfg_copy = copy.deepcopy(model_cfg)
        dataset_copy = copy.deepcopy(dataset)
        self.centerpoint_rcnn = CenterPointRCNN(model_cfg=model_cfg, num_class=num_class, dataset=dataset)

        self.centerpoint_rcnn_ema = CenterPointRCNN(model_cfg=model_cfg_copy, num_class=num_class, dataset=dataset_copy)
    ...
    def forward(self, batch_dict):
        if self.training:
            ...
            with torch.no_grad():
                ...
                pred_dicts, recall_dicts = self.centerpoint_rcnn_ema.post_processing(batch_dict_ema,
