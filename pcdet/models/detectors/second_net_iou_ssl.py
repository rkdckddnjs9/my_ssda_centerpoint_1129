...
class SECONDNetIoU_SSL(Detector3DTemplate):
    def __init__(self, model_cfg, num_class, dataset):
        super().__init__(model_cfg=model_cfg, num_class=num_class, dataset=dataset)
        # self.module_list = self.build_networks()
        model_cfg_copy = copy.deepcopy(model_cfg)
        dataset_copy = copy.deepcopy(dataset)
        self.second_iou = SECONDNetIoU(model_cfg=model_cfg, num_class=num_class, dataset=dataset)
        
        self.second_iou_ema = SECONDNetIoU(model_cfg=model_cfg_copy, num_class=num_class, dataset=dataset_copy)
        for param in self.second_iou_ema.parameters():
            param.detach_()
        self.add_module('second_iou', self.second_iou)
        self.add_module('second_iou_ema', self.second_iou_ema)
    ...
    def forward(self, batch_dict):
        if self.training:
            ...
            with torch.no_grad():
                ...
                for cur_module in self.second_iou_ema.module_list:
                    try:
                        batch_dict_ema = cur_module(batch_dict_ema, disable_gt_roi_when_pseudo_labeling=True)
                    except:
                        batch_dict_ema = cur_module(batch_dict_ema)
                pred_dicts, recall_dicts = self.second_iou_ema.post_processing(batch_dict_ema,
            ...
            for cur_module in self.second_iou.module_list:
                batch_dict = cur_module(batch_dict)

            disp_dict = {}
            loss_rpn, tb_dict = self.second_iou.dense_head.get_loss()
            # loss_point, tb_dict = self.pv_rcnn.point_head.get_loss(tb_dict, scalar=False)
            loss_rcnn, tb_dict = self.second_iou.roi_head.get_loss(tb_dict)
            ...
            pred_dicts, recall_dicts = self.second_iou.post_processing(batch_dict)
    ...
    def update_global_step(self):
        ...
        for ema_param, param in zip(self.second_iou_ema.parameters(), self.second_iou.parameters()):
