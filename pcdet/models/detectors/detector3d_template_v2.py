...
class Detector3DTemplateV2(nn.Module):
    ...
    def post_processing_ssl(self, batch_dict, no_recall_dict=False, override_thresh=None, no_nms=False):
        """
        Args:
            batch_dict:
                batch_size:
        ...
        for index in range(batch_size):
            ...
            if post_process_cfg.SSL_NMS_CONFIG.MULTI_CLASSES_NMS: #not
                if not isinstance(cls_preds, list):
                    #cls_preds = [cls_preds]
                    cls_preds = cls_preds
                    #multihead_label_mapping = [torch.arange(1, self.num_class, device=cls_preds[0].device)]
    ...
    def post_processing_for_roi(self, batch_dict):
        post_process_cfg = self.model_cfg.POST_PROCESSING
        batch_size = batch_dict['batch_size']
    ...
    def post_processing_for_roi_ssl(self, batch_dict):
        post_process_cfg = self.model_cfg.POST_PROCESSING
        batch_size = batch_dict['batch_size']
        #final_pred_dict = batch_dict['final_box_dicts']
        # final_pred_dict = [{'pred_boxes':batch_dict["rois"][i], 'pred_scores':batch_dict['roi_scores'][i], 'pred_labels':batch_dict['roi_labels'][i], 'pred_sem_scores':batch_dict['roi_ious'][i] } for i in range(batch_size)]
        final_pred_dict = [{'pred_boxes':batch_dict["rois"][i], 'pred_scores':batch_dict['roi_scores'][i], 'pred_labels':batch_dict['roi_labels'][i]} for i in range(batch_size)]
        #final_pred_dict = batch_dict['rois']
        recall_dict = {}
        for index in range(batch_size):
    ...
    def post_processing_for_refine_(self, batch_dict, no_recall_dict=True, override_thresh=None, no_nms=False):
        """
        Args:
            batch_dict:
                batch_size:
        ...
        for index in range(batch_size):
            ...
            if post_process_cfg.NMS_CONFIG.MULTI_CLASSES_NMS:
                ...
                for cur_cls_preds, cur_label_mapping in zip(cls_preds, multihead_label_mapping):
                    ...
                    #     score_thresh=post_process_cfg.SCORE_THRESH if override_thresh is None else override_thresh
                    # )
                    score, index = cur_cls_preds.max(axis=1)
                    cur_pred_labels = index + 1
    ...
    def post_processing_for_nuscenes(self, batch_dict):
        ...
        #final_pred_dict = [{'pred_boxes':batch_dict["rois"][i][non_zero_mask[i]], 'pred_scores':batch_dict['roi_scores'][i][non_zero_mask[i]], 'pred_labels':batch_dict['roi_labels'][i][non_zero_mask[i]]} for i in range(batch_size)]
