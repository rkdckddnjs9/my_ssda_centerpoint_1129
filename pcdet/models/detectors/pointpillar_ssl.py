...
class PointPillar_SSL(Detector3DTemplate):
    def __init__(self, model_cfg, num_class, dataset):
        super().__init__(model_cfg=model_cfg, num_class=num_class, dataset=dataset)
        model_cfg_copy = copy.deepcopy(model_cfg)
        dataset_copy = copy.deepcopy(dataset)
        self.pointpillar = PointPillar(model_cfg=model_cfg, num_class=num_class, dataset=dataset)

        self.pointpillar_ema = PointPillar(model_cfg=model_cfg_copy, num_class=num_class, dataset=dataset_copy)
        for param in self.pointpillar_ema.parameters():
            param.detach_()
        self.add_module('pointpillar', self.pointpillar)
        self.add_module('pointpillar_ema', self.pointpillar_ema)
    ...
    def forward(self, batch_dict):
        if self.training:
            ...
            with torch.no_grad():
                # self.pointpillar_ema.eval()  # Important! must be in train mode
                for cur_module in self.pointpillar_ema.module_list:
                    try:
                        batch_dict_ema = cur_module(batch_dict_ema, disable_gt_roi_when_pseudo_labeling=True)
                    except:
                        batch_dict_ema = cur_module(batch_dict_ema)
                pred_dicts, recall_dicts = self.pointpillar_ema.post_processing(batch_dict_ema,
            ...
            for cur_module in self.pointpillar.module_list:
                batch_dict = cur_module(batch_dict)

            disp_dict = {}
            loss_rpn_cls, loss_rpn_box, tb_dict = self.pointpillar.dense_head.get_loss()
            #loss_point, tb_dict = self.pv_rcnn.point_head.get_loss(tb_dict, scalar=False)
            loss_rcnn_cls, loss_rcnn_box, tb_dict = self.pointpillar.roi_head.get_loss(tb_dict, scalar=False)
